...
let owner,manager,backendWallet,ShipCoin,ShipCoinCurrency,ShipCoinStorage,ShipCoinBonusSystem,ShipCoinCrowdsale;

async function init() {
  //addWhiteList,changeMainWallet,getContributionInfoById,getContributorId,getUserPaymentById
  await ShipCoinStorage.addMultiOwnerTypes([1,3,4,5], manager).send({ from: owner, gas: 300000 });
  await ShipCoinStorage.addMultiOwnerTypes([1,3,4,5,11], backendWallet).send({ from: owner, gas: 300000 });


  //updateCurrency
  await ShipCoinCurrency.addMultiOwnerType(6, manager).send({ from: owner });
  await ShipCoinCurrency.addMultiOwnerType(6, backendWallet).send({ from: owner });
...
module.exports = async function(callback) {
  try {
    ...
    if(Contract.network == 'development') {
      ...
      backendWallet = '0x527b69E6F8Db9668C54f1769B8532F834E6fa2dD'
    }

    if(Contract.network == 'rinkeby') {
      owner = '0x49b7776eA56080439000fD54c45d72d3AC213020';
      manager = '0xaA92aD80969519b9061f4b73CcaEc340182271Bc';
      backendWallet = '0xF7f72A1DFF80E6799553912f09742810bee2FD3c';
      userInit.address = '0xF04fb691178B3F1b51ffe43ae4c258498aa10725'
    }
    ...
    if(argv.afterInit) {
      if(Contract.network == 'development') {
        ...
        await web3.eth.personal.unlockAccount(backendWallet,  "321321321", 15000);
        await web3.eth.personal.unlockAccount(userInit.address,    "123123123", 15000);
      } else if(Contract.network == 'rinkeby') {
        await web3.eth.personal.unlockAccount(manager,  "123123123", 15000);
        await web3.eth.personal.unlockAccount(backendWallet,  "123123123", 15000);
        await web3.eth.personal.unlockAccount(userInit.address,    "123123123", 15000);
      }
      switch (argv.afterInit) {
        case 1:
        case 'addWhiteList':
          if((await ShipCoinStorage.getContributionInfo(userInit.address).call({ from: backendWallet })).active == false) {
            await ShipCoinStorage.addWhiteList(userInit.id, userInit.address).send({ from: backendWallet, gas: 200000 });
            if((await ShipCoinStorage.getContributionInfo(userInit.address).call({ from: backendWallet })).active == true) {
        ...
        case 4:
        case 'init':
          await init(ShipCoinStorage,ShipCoinCrowdsale,ShipCoinCurrency,owner,manager,backendWallet);
          break;
