...
export function getCurrency(retInCents = false) {
  ...
  return fetch('https://api.coinmarketcap.com/v2/ticker/?limit=5')
    ...
    .then(result => {
      ...
      return fetch('https://api.coinmarketcap.com/v2/ticker/2081/');
    })
    .then(res => res.json())
    .then(result => {
      currency.amb = _get(result, 'data.quotes.USD.price', 0);
      if(retInCents) {
...
export function reCountUserPreSaleBonus(userData, minTotalUsdAmountInCents, bonusPercent, maxPayTime) {
  ...
  // userData.usdAbsRaisedInCents >= minTotalUsdAmountInCents
  if (userData.usdAbsRaisedInCents.gte(new BN(minTotalUsdAmountInCents))) {
    if (maxPayTime > 0) {
